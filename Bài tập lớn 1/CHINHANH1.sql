...
INSERT INTO  CHINHANH1.ORDER_DETAILS VALUES (20, 'P8', 1, 0);
...
WHERE WM.QUANTITY > 0;
    ...
    -- 1.5. Phép chia
    -- Tìm khách hàng có giới tính nữ đã mua tất cả sản phẩm có giá trên 15.000.000VNĐ vào ngày 20/10
    -- User : Employee
    -- Phạm vi: tại cửa hàng làm việc
...
WHERE C.CUS_GENDER = 'Nu'
  AND NOT EXISTS (
    SELECT P.PRODUCT_ID
    FROM CHINHANH1.PRODUCTS P
    WHERE P.PRODUCT_PRICE <= 15000000
      AND NOT EXISTS (
        ...
        WHERE O.CUS_ID = C.CUS_ID
          ...
          AND TO_DATE(O.ORDER_DATE, 'DD-MM-YYYY') = TO_DATE('20-10-2023', 'DD-MM-YYYY')
      )
  );
