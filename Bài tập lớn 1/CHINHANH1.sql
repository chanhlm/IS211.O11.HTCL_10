...
WHERE O.ORDER_DATE >= TO_DATE('05-01-2023', 'DD-MM-YYYY') AND WM.QUANTITY > 0;

        --Cho biết thông tin sản phẩm(PID, PNAME, PRICE) có số lượng bán nhiều nhất và có giá bán lớn hơn 10000000
        -- User : Employee
        -- Phạm vi: tại cửa hàng làm việc
...
WHERE WM.QUANTITY > 0;
    ...
    -- 1.5. Phép chia
    --Tìm tất khách hàng có giới tính nữ mua tất cả tất cả các sản phẩm từ số lượng 2 trở lên vào ngày 20/10
    -- User : Employee
    -- Phạm vi: tại cửa hàng làm việc
SELECT DISTINCT C.CUS_ID, C.CUS_NAME, C.CUS_GENDER
...
WHERE C.CUS_GENDER = 'Nu'
  AND NOT EXISTS (
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM CHINHANH1.ORDERS O
      JOIN CHINHANH1.ORDER_DETAILS OD ON O.ORDER_ID = OD.ORDER_ID
...
GROUP BY P.PRODUCT_ID, P.PRODUCT_NAME;
    ...
    -- 1.7. Tính toán: Liệt kê top 10 sản phẩm bán chạy trên các chi nhánh 
    -- User: DIRECTOR
    -- Phạm vi: toàn hệ thống
